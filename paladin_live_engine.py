...
class PaladinLiveEngine:
    ...
    def _loop_once(self) -> None:
        ...
        if now - self._last_hb_ts >= hb_sec:
            ...
            LOGGER.info(
                "PALADIN heartbeat | %s | elapsed=%ds left=%.0fs | mkt_mid up=%.4f dn=%.4f sum=%.4f "
                "| stagger=%s 1st_leg_mid<=%.3f pending=%s | pair_sum_max=%.3f live_2nd_mid_gate=%s "
                "hedge_timer_sum<=%s roi>=%.3f (~sum<=%.3f) "
                "| spent=$%.2f | inv UP=%.2fsh vwap=%.3f | inv DN=%.2fsh vwap=%.3f | "
                "pnl_if_up=$%.2f pnl_if_dn=$%.2f roi_u=%.4f roi_d=%.4f%s",
                slug,
                elapsed,
                secs_left,
                pm_u,
